...
## 📖 1. コンセプトと開発の原点： "Rusty" の哲学

本プロジェクトの原点は、映画『アイアンマン』に登場する究極のAIパートナー「J.A.R.V.I.S.」への憧れと挑戦です。クラウドの巨大なサーバーに依存せず、プライバシーが守られたローカル環境（エッジデバイス）で、主人の好みを理解し、ユーモアを交えて自律的にサポートする存在を目指しています。
...
OpenCV-Rust を利用し、カメラから取得した映像から主人の顔をリアルタイムで検知します。特徴量抽出によるローカル認識を行い、画像データは一切クラウドへ送信されません。主人の顔を検知した瞬間、記憶グラフ内の「主人に関連するノード」に重みが加算され、会話トーンがパーソナライズされます。
...
### 4.2 ソフトウェア要件
ホストOS (Raspberry Pi OS 64-bit 等) には、以下のツールのみインストールが必要です。
...
1. **リポジトリのクローンと環境変数設定**
   ```bash
   git clone https://github.com/yourusername/project-iris.git
   cd project-iris
   cp .env.example .env
   ```
